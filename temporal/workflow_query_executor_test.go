...
func (s *UnitTestSuite) TestQueryExecutorWorkflow() {
	...
	s.env.RegisterActivity(ac.WaitAndQuery)
	s.env.RegisterActivity(ac.DeleteNodes)
	...
	s.env.ExecuteWorkflow(QueryExecutor, QueryExecutorInput{
		NumNodes: 30,
		Query:    "SELECT sum(commits), event_type FROM github_events group by event_type",
		...
		// Query:       "select 1",
		KeeperHost:  "6e82576a050098.vm.test-bighouse-t-keeper.internal",
